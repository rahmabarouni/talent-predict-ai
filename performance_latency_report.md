...
*Generated automatically on: **2026-05-19 16:10:18***
...
| :--- | :--- | :--- | :--- | :--- |
| **Ollama** | Port `11434` | 🟢 ACTIVE | **6.31 ms** | **29.59 ms** |
| **AI Service** | Port `8000` | 🟢 ACTIVE | **0.66 ms** | **3.70 ms** |
| **CV Extractor** | Port `9000` | 🟢 ACTIVE | **0.63 ms** | **3.15 ms** |
| **n8n** | Port `5678` | 🟢 ACTIVE | **0.71 ms** | **4.35 ms** |
| **Backend (Spring)** | Port `8081` | 🔴 OFFLINE | *N/A (Offline)* | *N/A* |
...
| :--- | :--- | :--- | :--- | :--- |
| `talentpredict-ai` ➔ `cv-extractor` | **Container-to-Container (C2C)** | `http://cv-extractor:9000/health` | **13.09 ms** | Very low bridge overhead |
| `talentpredict-ai` ➔ Host gateway | **Container-to-Host (C2H)** | `http://host.docker.internal:11434` | **23.58 ms** | Negligible virtualization cost |

> [!NOTE]
> The container-to-container latency over the custom bridge `talentpredict-net` shows **extremely fast throughput** (~13.09ms) comparable to local system memory access, meaning the multi-container microservice layout is highly performant.
...
| :--- | :--- | :--- |
| **Prompt Request Roundtrip** | **436.87 ms** | Time taken for total HTTP request-response cycle |
| **Prompt Evaluation Latency** | **178.52 ms** | Time taken to ingest prompt text (41 tokens) |
| **Token Generation Latency** | **84.49 ms** | Time taken to synthesize output text (7 tokens) |
| **Model Generation Throughput** | **82.85 tokens/sec** | **Model throughput capability on current hardware** |
...
> [!TIP]
> A generation throughput of **82.85 tokens/second** indicates that AI assessments and career match operations will run very fast. For a standard 200-token question generation, expectation time is roughly **2.41 seconds**.
